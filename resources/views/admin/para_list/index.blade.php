                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="table-responsive table-body">
                                    <!--buttons_start-->
                                    <div class="col-md-12">
                                        <a href="{{url('admin/para_list/create')}}" class="btn btn-info "><i class="fa fa-align-center"></i>&nbsp;&nbsp;Add Question</a>
                                    ...
                                    <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                        @foreach($para_lists as $para_list)
                                            <tr>
                                                <td><input type="checkbox" value="{{$para_list->id}}" name="id[]" class="check"/></td>
                                                <td>{{$para_list->id}}</td>
                                                <td>{{$para_list->subject->sub_name}}</td>
                                                <td>{{$para_list->para_name}}</td>
                                                <td>{{$para_list->para_description}}</td>
                                              
                                                <td>{{$para_list->correct_ans}}</td>
                                                <td>{{$para_list->mark}}</td>
                                                <td>{{$para_list->negative_mark}}</td>
                                               
                                                
                                                <td>
                                                    <a href="{{url('admin/para_list/'.$para_list->id.'/edit')}}" class="btn btn-circle btn-lg btn-success icon-size">
                                                         <i class="fa fa-pencil-square-o"></i>
                                                    </a>
                                                    <span>
                                                    <form class="deleteForm" style="display:inline-block;" action="{{url('admin/para_list/'.$para_list->id)}}" method="post">
                                                    @csrf
                                                    @method('delete')
    ...
    $(document).ready(function(){
        ...
        function action(action){
            ...
            $('#example23 tbody .check:checked').each(function(){
                para_lists.push($(this).val());
            });

            $.ajax({
                type: 'POST',
                data:{ids:para_lists.join(','),action:action},
                url: '{{url("admin/para_list/action")}}',
