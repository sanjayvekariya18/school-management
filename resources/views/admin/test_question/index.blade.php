                    ...
                    <div class="col-12">
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <!-- ============================================================== -->
                                <h4 class="card-title">Test Question Detail</h4>
                                <h6 class="card-subtitle">Test Question Details Of Student</h6>
                                <div class="table-responsive table-body">
                                    <!--buttons_start-->
                                    <div class="col-md-12">
                                        <a href="{{url('admin/test_question/create')}}" class="btn btn-info "><i class="fa fa-align-center"></i>&nbsp;&nbsp;Add Test Question</a>
                                    ...
                                    <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        @foreach($test_questions as $test_question)
                                            <tr>
                                                <td><input type="checkbox" value="{{$test_question->id}}" name="id[]" class="check"/></td>
                                                <td>{{$test_question->id}}</td>
                                                <td>{{$test_question->test->test_name}}</td>
                                                <td>{{$test_question->context_id}}</td>
                                                <td>{{$test_question->question_type}}</td>
                                                <td>
                                                    <a href="{{url('admin/test_question/'.$test_question->id.'/edit')}}" class="btn btn-circle btn-lg btn-success icon-size">
                                                         <i class="fa fa-pencil-square-o"></i>
                                                    </a>
                                                    <span>
                                                    <form class="deleteForm" style="display:inline-block;" action="{{url('admin/test_question/'.$test_question->id)}}" method="post">
                                                    @csrf
                                                    @method('delete')
    ...
    $(document).ready(function(){
        ...
        function action(action){
            ...
            $('#example23 tbody .check:checked').each(function(){
                test_questions.push($(this).val());
            });

            $.ajax({
                type: 'POST',
                data:{ids:test_questions.join(','),action:action},
                url: '{{url("admin/test_question/action")}}',
