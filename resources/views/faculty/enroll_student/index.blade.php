                    ...
                    <div class="col-12">
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <!-- ============================================================== -->
                                <h4 class="card-title">Enrollment Detail</h4>
                                <h6 class="card-subtitle">Enrollment Details Of Student</h6>
                                <div class="table-responsive table-body">
                                    <!--buttons_start-->
                                    <div class="col-md-12">
                                        <a href="{{url('faculty/enroll_student/create')}}" class="btn btn-info "><i class="fa fa-align-center"></i>&nbsp;&nbsp;Add Enrollment</a>
                                    ...
                                    <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        @foreach($enroll_students as $enroll_student)
                                            <tr>
                                                <td><input type="checkbox" value="{{$enroll_student->id}}" name="id[]" class="check"/></td>
                                                <td>{{$enroll_student->id}}</td>
                                                <td>{{$enroll_student->student->middlename}}</td>
                                                <td>{{$enroll_student->stream->stream_name}}</td>
                                                <td>{{$enroll_student->semester->sem_name}}</td>
                                                <td>{{$enroll_student->divison->divison_name}}</td>
                                                <td>{{($enroll_student->status) ? "Active" : "Deactive" }}</td>
                                                <td>
                                                    <a href="{{url('faculty/enroll_student/'.$enroll_student->id.'/edit')}}" class="btn btn-circle btn-lg btn-success icon-size">
                                                         <i class="fa fa-pencil-square-o"></i>
                                                    </a>
                                                    <span>
                                                    <form class="deleteForm" style="display:inline-block;" action="{{url('faculty/enroll_student/'.$enroll_student->id)}}" method="post">
                                                    @csrf
                                                    @method('delete')
    ...
    $(document).ready(function(){
        ...
        function action(action){
            ...
            $('#example23 tbody .check:checked').each(function(){
                enroll_students.push($(this).val());
            });

            $.ajax({
                type: 'POST',
                data:{ids:enroll_students.join(','),action:action},
                url: '{{url("faculty/enroll_student/action")}}',
