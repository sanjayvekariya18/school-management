                    ...
                    <div class="col-12">
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <h4 class="card-title">Question Detail</h4>
                                <h6 class="card-subtitle">Question Details for Exam</h6>
                                <div class="table-responsive table-body">
                                    <!--buttons_start-->
                                    <div class="col-md-12">
                                        <a href="{{url('admin/tof_question/create')}}" class="btn btn-info "><i class="fa fa-align-center"></i>&nbsp;&nbsp;Add Question</a>
                                    ...
                                    <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                        @foreach($tof_questions as $tof_question)
                                            <tr>
                                                <td><input type="checkbox" value="{{$tof_question->id}}" name="id[]" class="check"/></td>
                                                <td>{{$tof_question->id}}</td>
                                                <td>{{$tof_question->subject->sub_name}}</td>
                                                <td>{{$tof_question->question}}</td>
                                                <td>{{$tof_question->correct_ans}}</td>
                                                <td>{{$tof_question->mark}}</td>
                                                <td>{{$tof_question->negative_mark}}</td>
                                               
                                                
                                                <td>
                                                    <a href="{{url('admin/tof_question/'.$tof_question->id.'/edit')}}" class="btn btn-circle btn-lg btn-success icon-size">
                                                         <i class="fa fa-pencil-square-o"></i>
                                                    </a>
                                                    <span>
                                                    <form class="deleteForm" style="display:inline-block;" action="{{url('admin/tof_question/'.$tof_question->id)}}" method="post">
                                                    @csrf
                                                    @method('delete')
    ...
    $(document).ready(function(){
        ...
        function action(action){
            ...
            $('#example23 tbody .check:checked').each(function(){
                tof_questions.push($(this).val());
            });

            $.ajax({
                type: 'POST',
                data:{ids:tof_questions.join(','),action:action},
                url: '{{url("admin/tof_question/action")}}',
