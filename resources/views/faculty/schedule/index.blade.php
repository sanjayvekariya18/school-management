                    ...
                    <div class="col-12">
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <!-- ============================================================== -->
                                <h4 class="card-title">Exam Schedule Detail</h4>
                                <h6 class="card-subtitle">Exam Schedule Details Of Student</h6>
                                <div class="table-responsive table-body">
                                    <!--buttons_start-->
                                    <div class="col-md-12">
                                        <a href="{{url('faculty/schedule/create')}}" class="btn btn-info "><i class="fa fa-align-center"></i>&nbsp;&nbsp;Add Schedule</a>
                                    ...
                                    <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        @foreach($schedules as $schedule)
                                            <tr>
                                                <td><input type="checkbox" value="{{$schedule->id}}" name="id[]" class="check"/></td>
                                                <td>{{$schedule->id}}</td>
                                                <td>{{$schedule->subject->sub_name}}</td>
                                                <td>{{$schedule->date}}</td>
                                                <td>{{$schedule->start_time}}</td>
                                                <td>{{$schedule->end_time}}</td>
                                                <td>{{$schedule->top_scorer}}</td>
                                                <td>{{$schedule->no_of_top_scorers}}</td>
                                                <td>
                                                    <a href="{{url('faculty/schedule/'.$schedule->id.'/edit')}}" class="btn btn-circle btn-lg btn-success icon-size">
                                                         <i class="fa fa-pencil-square-o"></i>
                                                    </a>
                                                    <span>
                                                    <form class="deleteForm" style="display:inline-block;" action="{{url('faculty/schedule/'.$schedule->id)}}" method="post">
                                                    @csrf
                                                    @method('delete')
...
<script>
    ...
    $(document).ready(function(){
        ...
        function action(action){
            ...
            $.ajax({
                type: 'POST',
                data:{ids:schedules.join(','),action:action},
                url: '{{url("faculty/schedule/action")}}',
