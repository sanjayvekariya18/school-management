                    ...
                    <div class="col-12">
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <!-- ============================================================== -->
                                <h4 class="card-title">Faculty Subject Detail</h4>
                                <h6 class="card-subtitle">Faculty Subject Details Of Collage</h6>
                                <div class="table-responsive table-body">
                                    <!--buttons_start-->
                                    <div class="col-md-12">
                                        <a href="{{url('faculty/faculty_subject/create')}}" class="btn btn-info "><i class="fa fa-align-center"></i>&nbsp;&nbsp;Add Faculty Subject</a>
                                        <button type="button" class="btn btn-success action" value="Active"><i class="fa fa-eye"></i>&nbsp;&nbsp;Active</button>
                                        <button type="button" class="btn btn-primary action" value="Deactive"><i class="fa fa-eye-slash"></i>&nbsp;&nbsp;Deactive</button>
                                    ...
                                    <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        @foreach($faculty_subjects as $faculty_subject)
                                            <tr>
                                                <td><input type="checkbox" value="{{$faculty_subject->id}}" name="id[]" class="check"/></td>
                                                <td>{{$faculty_subject->id}}</td>
                                                <td>{{$faculty_subject->faculty->fac_name}}</td>
                                                <td>{{$faculty_subject->stream->stream_name}}</td>
                                                <td>{{$faculty_subject->semester->sem_name}}</td>
                                                <td>{{$faculty_subject->divison->divison_name}}</td>
                                                <td>{{$faculty_subject->subject->sub_name}}</td>
                                                <td>{{($faculty_subject->status) ? "Active" : "Deactive" }}</td>
                                                <td>
                                                    <a href="{{url('faculty/faculty_subject/'.$faculty_subject->id.'/edit')}}" class="btn btn-circle btn-lg btn-success icon-size">
                                                         <i class="fa fa-pencil-square-o"></i>
                                                    </a>
                                                    <span>
                                                    <form class="deleteForm" style="display:inline-block;" action="{{url('faculty/faculty_subject/'.$faculty_subject->id)}}" method="post">
                                                    @csrf
                                                    @method('delete')
...
$(document).on('click','.deleteBtn',function(e){
    ...
    $(document).ready(function(){
        ...
        function action(action){
            $('.loader').show();
            var faculty_subjects = [];
            $('#example23 tbody .check:checked').each(function(){
                faculty_subjects.push($(this).val());
            });

            $.ajax({
                type: 'POST',
                data:{ids:faculty_subjects.join(','),action:action},
                url: '{{url("faculty/faculty_subject/action")}}',
