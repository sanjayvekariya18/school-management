                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="table-responsive table-body">
                                    <!--buttons_start-->
                                    <div class="col-md-12">
                                        <a href="{{url('faculty/divison/create')}}" class="btn btn-info "><i class="fa fa-align-center"></i>&nbsp;&nbsp;Add Divison</a>
                                    ...
                                    <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        @foreach($divisons as $divison)
                                            <tr>
                                                <td><input type="checkbox" value="{{$divison->id}}" name="id[]" class="check"/></td>
                                                <td>{{$divison->id}}</td>
                                                <td>{{$divison->semester->sem_name}}</td>
                                                <td>{{$divison->divison_name}}</td>
                                                <td>
                                                    <a href="{{url('faculty/divison/'.$divison->id.'/edit')}}" class="btn btn-circle btn-lg btn-success icon-size">
                                                         <i class="fa fa-pencil-square-o"></i>
                                                    </a>
                                                    <span>
                                                    <form class="deleteForm" style="display:inline-block;" action="{{url('faculty/divison/'.$divison->id)}}" method="post">
                                                    @csrf
                                                    @method('delete')
    ...
    $(document).ready(function(){
        ...
        function action(action){
            ...
            $('#example23 tbody .check:checked').each(function(){
                divisons.push($(this).val());
            });

            $.ajax({
                type: 'POST',
                data:{ids:divisons.join(','),action:action},
                url: '{{url("faculty/divison/action")}}',
